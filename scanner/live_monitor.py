...
from config.paths import CANDLE_CACHE_FILE, ACTIVE_SYMBOLS_FILE
...
class LiveMarketMonitor:
    ...
    def calculate_opportunity_score(self, snapshot: MarketSnapshot) -> float:
        ...
        atr_score = min(100, snapshot.atr_pct * 100 * 30)

        # Volume score (target: $100k+)
        vol_score = min(100, snapshot.volume_24h / 5000)

        # Wick size score
        wick_size_score = min(100, snapshot.avg_wick_size_pct * 100 * 15)
    ...
    def print_status(self, snapshots: List[MarketSnapshot]):
        ...
        for sym in self.current_symbols:
            snap = self.market_snapshots.get(sym)
            pinned = "[P]" if sym in self.pinned_symbols else "   "
            if snap:
                trend = "UP" if snap.score_change > 0 else "DN" if snap.score_change < 0 else "--"
        ...
        while True:
            try:
                ...
                for alert in alerts:
                    ...
                    if console_alerts:
                        priority_icon = {"low": "i", "normal": "*", "high": "!", "urgent": "!!!"}.get(alert.priority, "*")
